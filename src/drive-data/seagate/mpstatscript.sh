...
version=1.0
# periodically collects mpstat samples

# Version 1.0, February 2016, based on statsscript.sh v1.3
# Samples only mpstat and uses the mpstat interval and count parameters vs looping count X delay
...
#Collects interval count samples from each stat with interval_duration between samples
    ...
    outfile=$sampledir"/"$tracedt$flname"_period"$((period++))"of${runperiods}_mpstat.txt"
    logmessage "$(basename $0), Logging to $outfile"
#    local cnt=$interval_count
#    while [ $cnt -gt 0 ]; do
#        ${IOSTAT_BIN} -mtxy $device >> ${outfile}_iostat.txt
        ${MPSTAT_BIN} -P ALL $interval_duration $interval_count >> $outfile

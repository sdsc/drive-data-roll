...
declare filebase="$appname_jobid_`uname -n`"
declare folder="results/$TESTdt_$filebase"
...
for ((ndx=0; ndx < ${#_drives[@]}; ndx++));do
  ...
  local SCSIt=`ls -ls /sys/class/scsi_generic | $AWKBIN -F/ '{if ($11 == '$drive') print $8}'`
  ...
  $_drives[$ndx]="/dev/"`ls -ls /sys/dev/block | $AWKBIN -F/ '{if ($8 == '$SCSIt') print $11}'`
  logadd "/dev/$drive = ${_drives[$ndx]}"
done
...
'./blktrscript.sh "'${_drives[@]}'" '"$runsecs $tracedwell $sampleperiodsecs $firstwaitsecs $padsecs $folder $filebase" \
'./statsscript.sh "'${_drives[@]}'" '"$runsecs $tracedwell $sampleperiodsecs $firstwaitsecs $padsecs $folder $filebase")
#Array for scripts to run at end. Can be added to. Run sequentially in this thread.
declare -a _scriptsatend=("./stxm2.sh")

#Kick off sampling scripts in separate threads and return
if [ $func == "START" ];then

  for script in ${_scriptstostart[@]};do
...
if [ $func == "END" ];then
  ...
  #Run end scripts
  for script in ${_scriptsatend[@]};do
